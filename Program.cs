        ...
        static async Task Main(string[] args)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    _ = Task.Run(async () => ClientHandler(tcpClient, mainServer)); 
                }
            }
            catch (Exception ex)
            {
            ...
            {
                tcpListener.Stop();
                Console.WriteLine($"Сервер завершил работу ({DateTime.UtcNow}) {ipAddress.ToString()}:{port}");
            }
        }
        static async Task ClientHandler(TcpClient tcpClient, MainServer mainServer)
        {
            try
            {
